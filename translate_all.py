#!/usr/bin/env python3
"""아르노서지 DX 한국어 패치 전체 빌드 도구.

translations/romfs의 한국어 EBM과 시스템 메시지 XML을 한 번에 변환하고,
...
SYSTEM_MESSAGES = TRANSLATIONS / "romfs" / "Saves" / "systemMessage"
...
def main():
    ...
    run([
        ...
        "--input", str(SYSTEM_MESSAGES),
        "--mapping", str(BUILD_REPORT),
        "--output", str(ATMOSPHERE / "romfs" / "Saves" / "systemMessage"),
    ])
