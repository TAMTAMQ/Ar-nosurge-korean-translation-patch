...
패치를 적용해 플레이하는 데에는 필요 없는 내용입니다. 어떤 제약이 있었고 그것을
어떻게 처리했는지, 어떤 도구로 무엇을 만들었는지를 남겨 둔 기록입니다.
이용자용 안내는 [README.md](README.md)에 있습니다.
...
README 의 "알려진 제약"에 적은 것들이 왜 그렇게 됐는지에 대한 기록이다.
...
`Saves/*.xml.e` 는 원문에 일본어가 있던 속성값 14,499개 중 번역이 필요한 XML 이 25개였고,
나머지 167개는 일본어 텍스트가 없어 저장소에서 제외했다. 미번역으로 남은 것은 UI 레이아웃
치수 측정용 더미 문자열(`あいうえお…`)과 아티스트 표기(`Origa・dottedline hiroko`)뿐이다.

폰트는 희귀 한자 영역을 한글용으로 빌려 쓴다. 확인된 시마법 선택 화면의 동적 일본어 문자는
보호 목록에 넣어 제외했지만, 아직 확인하지 못한 UI 에서 같은 충돌이 나타날 수 있다.
...
이 중 `mov w1, w21` 을 `mov w1, wzr`(0) 로 바꿔 대기를 없앤다. 실제 변경은 1바이트이고
IPS 파일 전체가 14바이트다. 빌드 ID 로 대상을 확인하므로 버전이 다르면 적용되지 않는다.
