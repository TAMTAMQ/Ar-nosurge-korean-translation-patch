...
RULES = [
    ...
    {"jp": "アーシェス", "not_jp": None,
     "sources": ["아르 시엘", "아르시엘", "아르셰스", "아스헤스"],
     "target": "아셰스"},
    # イオナサル(본명) 과 イオン(애칭) 은 같은 인물이다 - "イオンって呼んでね".
    # 둘이 같이 나오는 줄은 원문이 두 이름을 구분해 쓰고 있으므로 건드리지 않는다.
    # `이오나사` 는 넣으면 안 된다. イオナサブレ(과자 이름) 가 `이오나사르브레` 가
    # 되어버린다. 조사까지 통째로 짚어야 `이오나사의` 의 `의` 도 살아남는다.
    {"jp": "イオナサル", "not_jp": "イオン",
     "sources": ["이오나사의"], "target": "이오나사르의"},
    {"jp": "イオナサル", "not_jp": "イオン",
     "sources": ["이오나사루", "이온살", "이온사르", "이온"],
     "target": "이오나사르"},
    {"jp": "カノイール", "not_jp": None,
     "sources": ["카누아르", "카노이"], "target": "카노일"},
...
def swap(text, source, target, batchim_of=None):

    """표기를 바꾸고, 받침 유무가 달라지면 뒤따르는 조사도 맞춘다.

    target 이 자리표시자일 수 있으므로, 받침 판정에 쓸 글자를 따로 받는다.
    ...
    reference = batchim_of if batchim_of is not None else target

    was, now = has_batchim(source[-1]), has_batchim(reference[-1])
...
def unify(japanese, korean):
    ...
    for rule in RULES:
        ...
        # 이미 확정 표기로 된 자리를 먼저 가린다. `이오나사` 가 `이오나사르` 의

        # 앞부분이라 그냥 치환하면 `이오나사르르` 가 된다.

        text = text.replace(target, MASK)

        # 긴 표기부터 바꿔야 짧은 표기가 긴 표기를 잡아먹지 않는다.

        for source in sorted(rule["sources"], key=len, reverse=True):

            if source == target:

                continue

            text = swap(text, source, MASK, batchim_of=target)

        text = text.replace(MASK, target)
