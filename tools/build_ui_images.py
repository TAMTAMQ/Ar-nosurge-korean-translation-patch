#!/usr/bin/env python3
"""직접 번역한 텍스처(`translateImage/*.g1t`)를 설치 폴더에 넣는다.

메뉴·시스템 화면의 일본어 상당수는 텍스트가 아니라 텍스처에 그려져 있어서
텍스트 패치로는 손댈 수 없다. 그 부분은 GIMP로 직접 고쳐 `translateImage/`에
`.g1t` 로 두고, 이 단계에서 원본 위치로 복사한다. 경고 화면
`warning_text.g1t`는 `romfs/Data/NX/Ipu/`, 나머지 UI는 `romfs/Data/NX/ui/`이다.
...
def main():
    ...
    parser.add_argument("--original", type=Path,
                        help="언팩된 romfs의 Data/NX/ui 폴더. Ipu는 형제 폴더를 대조한다.")
    parser.add_argument("--output", type=Path, required=True,
                        help="설치용 romfs 폴더 (Data/NX/ui 또는 Ipu를 그 아래에 만든다)")
    ...
    for image in images:
        ...
        is_warning = image.name.lower() == "warning_text.g1t"
        subpath = IPU_SUBPATH if is_warning else UI_SUBPATH
        if args.original:
            original_dir = args.original.parent / "Ipu" if is_warning else args.original
            source = original_dir / image.name
        ...
        destination = args.output / subpath / image.name
        destination.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(image, destination)
        print(f"built: {destination} ({len(data):,}바이트)")
    print(f"번역 텍스처 {len(images)}개 적용")
